...
let colorArr = ['table-active','table-secondary','table-light'];
...
$(document).on("change", "#sido", function() {
	$.get("/map/gugun"
			...
			,function(data, status){
				$("#gugun").empty();
				$("#gugun").append('<option value="all">시/구/군</option>');
				$.each(data, function(index, vo) {
...
$(document).on("change", "#gugun", function() {
	$.get("/map/dong"
			...
			,function(data, status){
				$("#dong").empty();
				$("#dong").append('<option value="all">동</option>');
				$.each(data, function(index, vo) {
...
$(function () {
    ...
    var options = { // 지도를 생성할 때 필요한 기본 옵션
    		center: new kakao.maps.LatLng(35.19379469896311, 129.06118980960358), // 지도의
